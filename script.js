...
function sportEventsToItems(evs){
  return (evs||[]).map(function(ev){
    ...
    var dateISO=ev.date.getFullYear()+'-'+String(ev.date.getMonth()+1).padStart(2,'0')+'-'+String(ev.date.getDate()).padStart(2,'0');
...
function getSportCategory(item){
  ...
  if(s.indexOf('voetbal')!==-1||sub.indexOf('league')!==-1||sub.indexOf('liga')!==-1)return 'voetbal';
...
function crowHtml(item){
  ...
  if(item._type==='live'){
    ...
    if(item._isLiveNow)epBadge='<div class="crow-ep-badge" style="background:rgba(232,54,74,0.12);color:#e8364a">LIVE</div>';
    else if(timeInfo)epBadge='<div class="crow-ep-badge">'+timeInfo+'</div>';
  }
