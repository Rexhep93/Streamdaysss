/* ═══════════════════════════════════════════════════════════════
   ...
   No emojis. Real sport logos. Auto theme. Jost font.
   ═══════════════════════════════════════════════════════════════ */
...
function fetchTMDBReleases(){
  ...
  function makeItem(m,prov,id,type){var date=type==='movie'?(m.release_date||''):(m.first_air_date||'');if(!date||!m.id)return null;return{id:'tmdb-'+m.id+'-'+id,title:m.title||m.name||m.original_title||m.original_name||'',img:m.poster_path?TMDB_IMG+m.poster_path:null,_type:type,_date:date,_src:{name:prov.name,logo_100px:prov.logo},_style:{color:prov.color,text:prov.text},_key:providerKey(prov.name),_originType:detectOrigin(prov.name),_source:'tmdb',_providerId:Number(id),tmdb_id:m.id,user_rating:m.vote_average||0,overview:m.overview||''};}
...
function mergeItems(wm,tm){
  var result=[],seen={};wm.forEach(function(i){if(!i._date)return;var k=(i.title||'').toLowerCase().trim()+'|'+i._key;if(seen[k])return;seen[k]=true;result.push(i);});
  var ts={};tm.forEach(function(i){if(!i._date||!i.title)return;var dk=i.tmdb_id+'|'+i._key;if(ts[dk])return;ts[dk]=true;var tk=(i.title||'').toLowerCase().trim()+'|'+i._key;if(seen[tk])return;seen[tk]=true;result.push(i);});return result;
}
...
function buildDateTabs(){
  ...
  var itemDays={};allItems.forEach(function(it){if(it._date)itemDays[it._date]=true;});
  Object.keys(itemDays).forEach(function(d){if(allDays.indexOf(d)===-1)allDays.push(d);});allDays.sort();
...
function crowHtml(item){
  var title=(item.title||'').replace(/'/g,"&#39;").replace(/"/g,'&quot;');var sid=String(item.id).replace(/['"\\]/g,'');var poster=imgCache[item.id]||item.img||'';var tl=item._type==='movie'?'Film':'Serie';var cls=item._type==='movie'?'film':'serie';var season=(item._type==='tv'&&item._season)?' - S'+item._season:'';var sn=(item._src&&item._src.name||'Streaming').replace(/Netherlands/gi,'').trim();
...
function setType(f){
  ...
  var dt=document.getElementById('dateTabs');var sb=document.getElementById('svcBar');var sl=document.querySelector('.slogan-bar');
  if(f==='top10'){dt.style.display='none';sb.style.display='none';if(sl)sl.style.display='none';renderTop10();}
  else if(f==='livesport'){dt.style.display='none';sb.style.display='none';if(sl)sl.style.display='none';renderLiveSport();}
  else if(f==='search'){dt.style.display='none';sb.style.display='none';if(sl)sl.style.display='none';renderSearch();}
  else{dt.style.display='';sb.style.display='';if(sl)sl.style.display='';renderMain();}
}
...
function openModal(rawId){
  ...
  chain.then(function(){
    ...
    si.innerHTML='<div class="sheet-header"><div class="sp">'+(posterUrl?'<img src="'+posterUrl+'" alt="">':'')+'</div><div class="sinf"><div class="ssvc" style="color:'+color+'">'+svcName+'</div><div class="stitle">'+fullTitle+'</div><div class="smeta">'+mp.join(' - ')+'</div><div class="stags">'+genres+'</div>'+oBadge+'</div></div>';
    ...
    if(watchLink){wb.href=watchLink;wb.style.display='flex';var wbLogo=document.getElementById('wbLogo');var wbLabel=document.getElementById('wbLabel');var pLogo=item._src&&item._src.logo_100px||null;if(pLogo&&wbLogo){wbLogo.src=pLogo;wbLogo.alt=svcName;wbLogo.style.display='block';}else if(wbLogo){wbLogo.style.display='none';}if(wbLabel)wbLabel.textContent='Kijken op '+svcName;}
  });
}
...
function fetchTop10(p,c){var ck='t10_'+p+'_'+c;if(top10Cache[ck])return Promise.resolve(top10Cache[ck]);var ep=c==='all'?'/trending/all/'+p:'/trending/'+c+'/'+p;return tmdb(ep,{language:'nl-NL',region:'NL'}).then(function(d){var r=(d.results||[]).slice(0,10);top10Cache[ck]=r;return r;}).catch(function(){return[];});}
...
var SPORT_STREAMERS={ESPN:{bg:'rgba(204,0,0,0.85)'},Viaplay:{bg:'rgba(123,79,227,0.85)'},'Ziggo Sport':{bg:'rgba(255,107,0,0.85)'},'Canal+':{bg:'rgba(70,70,70,0.85)'},'Discovery+':{bg:'rgba(0,54,160,0.85)'},Netflix:{bg:'rgba(229,9,20,0.85)'},'Prime Video':{bg:'rgba(0,168,224,0.85)'}};
...
function fetchSportEvents(){
  ...
  var ffs=FOOTBALL_COMPS.map(function(comp){return fd('/competitions/'+comp.code+'/matches',{dateFrom:dateFrom,dateTo:dateTo}).then(function(data){(data.matches||[]).forEach(function(m){if(!m.utcDate)return;if(['FINISHED','AWARDED','CANCELLED','POSTPONED','SUSPENDED'].indexOf(m.status)!==-1)return;var d=new Date(m.utcDate);if(isNaN(d.getTime()))return;var home=m.homeTeam&&(m.homeTeam.shortName||m.homeTeam.name)||'?';var away=m.awayTeam&&(m.awayTeam.shortName||m.awayTeam.name)||'?';var str=comp.streamer;if(comp.code==='PL'&&comp.plStreamer&&PL_PRIME_MATCHDAYS[m.matchday])str=comp.plStreamer;var live=['IN_PLAY','PAUSED','LIVE'].indexOf(m.status)!==-1;add({id:'fd-'+m.id,title:home+' - '+away,subtitle:comp.name,sport:'Voetbal',streamer:str,date:d,venue:m.venue||'',isLive:live});});}).catch(function(e){sportFetchErrors.push(comp.name+': '+e.message);});});
  ...
  add({id:'wwe-raw',title:'WWE Raw',subtitle:'WWE',sport:'Wrestling',streamer:'Netflix',date:nextWd(1,2,0),venue:'Netflix NL',isLive:false});
  add({id:'wwe-sd',title:'WWE SmackDown',subtitle:'WWE',sport:'Wrestling',streamer:'Netflix',date:nextWd(5,2,0),venue:'Netflix NL',isLive:false});
  add({id:'ufc',title:'UFC Fight Night',subtitle:'UFC',sport:'MMA',streamer:'Discovery+',date:nextWd(6,5,0),venue:'Discovery+ NL',isLive:false});
...
function doSearch(q){
  ...
  var localHits=allItems.filter(function(i){return i.title&&i.title.toLowerCase().indexOf(ql)!==-1;});
  ...
  /* Also search TMDB for extra results */
  Promise.all([
    ...
    var tmdbHits=[];
    (res[0].results||[]).slice(0,8).forEach(function(m){
      var k=(m.title||m.name||'').toLowerCase()+'|movie';
      if(!seen[k]){seen[k]=true;tmdbHits.push({id:'search-m-'+m.id,title:m.title||m.name||'',img:m.poster_path?TMDB_IMG+m.poster_path:null,_type:'movie',_date:m.release_date||'',_src:{name:'Zoekresultaat'},_style:{color:'#40e86a',text:'#fff'},_key:'search',_originType:'licensed',_source:'tmdb',tmdb_id:m.id,overview:m.overview||'',user_rating:m.vote_average||0});}
    });
    (res[1].results||[]).slice(0,8).forEach(function(t){
      var k=(t.name||t.title||'').toLowerCase()+'|tv';
      if(!seen[k]){seen[k]=true;tmdbHits.push({id:'search-t-'+t.id,title:t.name||t.title||'',img:t.poster_path?TMDB_IMG+t.poster_path:null,_type:'tv',_date:t.first_air_date||'',_src:{name:'Zoekresultaat'},_style:{color:'#40e86a',text:'#fff'},_key:'search',_originType:'licensed',_source:'tmdb',tmdb_id:t.id,overview:t.overview||'',user_rating:t.vote_average||0});}
    });
    /* Add TMDB hits to allItems so modal works */
    tmdbHits.forEach(function(h){if(!allItems.find(function(i){return i.id===h.id;}))allItems.push(h);});
    var combined=deduped.concat(tmdbHits).slice(0,20);
    ...
    el.innerHTML=combined.map(function(item){
      ...
      var sn=(item._src&&item._src.name||'').replace(/Netherlands/gi,'').replace(/Zoekresultaat/gi,'').trim();
      var metaParts=[tl];if(year)metaParts.push(year);if(sn)metaParts.push(sn);
      return '<div class="crow" data-id="'+sid+'"><div class="crow-poster">'+(poster?'<img src="'+poster+'" alt="'+title+'" loading="lazy">':'<div class="crow-fb">'+title+'</div>')+'</div><div class="crow-info"><div class="crow-title">'+title+'</div><div class="crow-meta">'+metaParts.join(' - ')+'</div></div><div class="crow-badge '+cls+'">'+tl+'</div></div>';
    }).join('');
  });
}
...
function init(){
  ...
  Promise.all([fetchWMSources(),fetchTMDBProviders().catch(function(){})]).then(function(){
    var tc=Object.keys(TMDB_NL_PROVIDERS).length;if(lt)lt.textContent='Releases laden...';if(ls)ls.textContent=wmSources.length+' WM + '+tc+' TMDB diensten';
    return Promise.all([fetchWMReleases().catch(function(){return[];}),fetchTMDBReleases().catch(function(){return[];})]);
  }).then(function(res){if(ls)ls.textContent='';allItems=mergeItems(res[0],res[1]);console.log('[StreamGids] '+allItems.length+' items');if(!allItems.length)throw new Error('Geen releases gevonden.');buildSvcBar();buildDateTabs();renderMain();enrichMissingPosters();}).catch(function(e){console.error(e);document.getElementById('main').innerHTML='<div class="error-screen"><div class="error-title">Kon niet laden</div><div class="error-msg">'+(e.message||'Controleer je verbinding.')+'</div><button class="retry-btn" onclick="location.reload()">Opnieuw</button></div>';});
}
