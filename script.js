...
function goHome(){
  selectedDate=todayISO();typeFilter='all';svcFilter='all';activeFilter=null;
  ...
  buildDateTabs();renderMain();window.scrollTo({top:0,behavior:'smooth'});
}
...
function toggleFilter(f){
  if(activeFilter===f)activeFilter=null;else activeFilter=f;
...
function updateFilterChips(f){
  var epChip=document.querySelector('[data-filter="episodes"]');var seaChip=document.querySelector('[data-filter="seasons"]');
  if(f==='movie'){if(epChip)epChip.style.display='none';if(seaChip)seaChip.style.display='none';if(activeFilter==='episodes'||activeFilter==='seasons'){activeFilter=null;document.querySelectorAll('.filter-chip').forEach(function(c){c.classList.remove('active');});}}
  else{if(epChip)epChip.style.display='';if(seaChip)seaChip.style.display='';}
}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if(filterBar){filterBar.addEventListener('click',function(e){var chip=e.target.closest('.filter-chip');if(chip)toggleFilter(chip.getAttribute('data-filter'));});}
...
function init(){
  ...
  function tryRender(){
    ...
    if(!hasRendered){hasRendered=true;buildSvcBar();buildDateTabs();renderMain();}
