...
var SPORT_LOGOS = {
  ...
  'Tennis':           'https://upload.wikimedia.org/wikipedia/commons/thumb/4/41/ATP_Tour_logo.svg/120px-ATP_Tour_logo.svg.png',
  'Darts':            'https://upload.wikimedia.org/wikipedia/en/thumb/f/fa/PDC_logo.svg/120px-PDC_logo.svg.png',
  'Award Show':       '',
  'Live Event':       '',
};
...
function matchProvider(n){
  ...
  /* FIX: Netflix Kids → Netflix */
  if(l==='netflix kids'||l==='netflix basic with ads')l='netflix';
...
/* ── Navigate home: today's releases ── */
function goHome(){
  selectedDate=todayISO();
  typeFilter='all';
  svcFilter='all';
  activeFilter=null;
  document.querySelectorAll('.bnav').forEach(function(n){n.classList.toggle('active',n.getAttribute('data-f')==='all');});
...
/* ══════════════════════════════════════════════════════════
   MERGE ITEMS — Multi-provider dedup:
   Show each title only ONCE (on the preferred/first provider),
   but track which other providers also have it.
   
   Google Sheet items for TV series are KEPT even if the API
   already has the same title+provider — they represent weekly
   episode releases that should keep showing up.
   ══════════════════════════════════════════════════════════ */
function mergeItems(wm,tm,gs){
  var result=[];
  /* titleProvMap: lowercase title → { item, providers: [key,...] } */
  var titleProvMap={};
  /* titleKeyMap: title|key → true (exact dedup) */
  var seen={};

  function addItem(i,source){
    if(!i._date||!i.title)return;
    var tl=(i.title||'').toLowerCase().trim();
    var tk=tl+'|'+i._key;
    if(seen[tk])return;
    seen[tk]=true;
    if(titleProvMap[tl]){
      /* Same title already exists on another provider — just add this provider name */
      var existing=titleProvMap[tl];
      if(existing.providers.indexOf(i._key)===-1){
        existing.providers.push(i._key);
        /* Store provider display names */
        if(!existing.item._alsoOn)existing.item._alsoOn=[];
        var dispName=(i._src&&i._src.name||i._key).replace(/Netherlands/gi,'').trim();
        if(existing.item._alsoOn.indexOf(dispName)===-1)existing.item._alsoOn.push(dispName);
      }
      return;
    }
    titleProvMap[tl]={item:i,providers:[i._key]};
    result.push(i);
  }

  wm.forEach(function(i){addItem(i,'wm');});
  tm.forEach(function(i){addItem(i,'tmdb');});

  /* Google Sheet items: for TV series, ALWAYS add (weekly episodes).
     For movies, only add if not already present on same provider. */
  (gs||[]).forEach(function(i){
    if(!i._date||!i.title)return;
    var tl=(i.title||'').toLowerCase().trim();
    var tk=tl+'|'+i._key;
    if(i._type==='tv'){
      /* For TV: allow if same title+key+date is unique (weekly releases) */
      var dateKey=tl+'|'+i._key+'|'+i._date;
      if(seen[dateKey])return;
      seen[dateKey]=true;
      /* If there's already a version of this title from API, copy poster etc */
      if(titleProvMap[tl]){
        var existing=titleProvMap[tl].item;
        if(!i.img&&existing.img){i.img=existing.img;imgCache[i.id]=existing.img;}
        if(!i.overview&&existing.overview)i.overview=existing.overview;
        if(!i.tmdb_id&&existing.tmdb_id)i.tmdb_id=existing.tmdb_id;
      }
      result.push(i);
    }else{
      /* Movie: normal dedup */
...
function renderMain(){
  ...
  if(!items.length){
    ...
    ctas+='<button class="empty-cta-btn" onclick="setType(\'top10\')">Bekijk Top 10</button>';
    ctas+='</div>';
    main.innerHTML='<div class="empty-state"><div style="font-size:16px;margin-bottom:6px;font-weight:600">Geen releases</div><div style="font-size:13px;color:var(--t3);margin-bottom:14px">Geen nieuwe content op '+getDateLabel(selectedDate).toLowerCase()+'. Probeer een andere dag.</div>'+ctas+'</div>';
    return;
  }
...
function crowHtml(item){
  ...
  /* Episode/season badges — show BOTH if it's episode 1 of a new season */
  var epBadge='';
  if(item._type==='tv'){
    var isNewSeason=!!item._season;
    var isNewEpisode=!!(item._epInfo&&item._epInfo.s);
    var isFirstEpOfSeason=isNewEpisode&&item._epInfo.e===1;
    var isGoogleSheet=item._source==='googlesheet';

    if(isFirstEpOfSeason||isNewSeason){
      /* Both: new season + new episode */
      var seasonNum=item._season||( item._epInfo?item._epInfo.s:null);
      epBadge='<div class="crow-ep-badge season">Nieuw seizoen'+(seasonNum?' (S'+seasonNum+')':'')+'</div>';
  ...
  /* "Ook te zien op" in meta */
  var alsoOnHtml='';
  if(item._alsoOn&&item._alsoOn.length>0){
    alsoOnHtml=' · Ook op '+item._alsoOn.join(', ');
  }
...
/* ── TOP 10 ── */
/* Fetches more than 10 trending items from TMDB, filters out
   those NOT on NL streaming, then fills to exactly 10. */
...
function fetchTop10(p,c){
  ...
  /* Fetch extra pages to have enough after filtering */
  return Promise.all([
    ...
    var r=[];
    pages.forEach(function(d){r=r.concat(d.results||[]);});
    /* Deduplicate by id */
    var seen={};r=r.filter(function(x){if(seen[x.id])return false;seen[x.id]=true;return true;});
    top10Cache[ck]=r.slice(0,60);
    return top10Cache[ck];
  });
}
function renderTop10(){
  ...
  fetchTop10(top10Period,top10Category).then(function(items){
    ...
    /* Check NL streaming for ALL items (up to 60), fill to 10 streamable */
    ...
    Promise.all(checks).then(function(){
      ...
      if(!streamable.length){
        el.innerHTML='<div class="sp-empty">Geen streambare content gevonden.<div class="empty-cta-row" style="margin-top:12px"><button class="empty-cta-btn" onclick="setType(\'all\')">Bekijk releases</button></div></div>';
        return;
      }
      /* Take exactly 10 */
      var top=streamable.slice(0,10);
      el.innerHTML=top.map(function(item,idx){
...
function fetchSportEvents(){
  ...
  /* Football from football-data.org */
  ...
  /* WWE on Netflix NL — recurring weekly */
  ...
  /* Prime Video sport — Champions League Tuesday & Wednesday matches */
  var clTue=nextWd(2,21,0);
  var clWed=nextWd(3,21,0);
  add({id:'prime-cl-tue',title:'Champions League Live',subtitle:'Champions League',sport:'Voetbal',streamer:'Prime Video',date:clTue,venue:'Geselecteerde wedstrijd',isLive:false});
  add({id:'prime-cl-wed',title:'Champions League Live',subtitle:'Champions League',sport:'Voetbal',streamer:'Prime Video',date:clWed,venue:'Geselecteerde wedstrijd',isLive:false});

  /* Tennis on Discovery+ */
  add({id:'tennis-disc',title:'ATP / WTA Tour',subtitle:'Tennis',sport:'Tennis',streamer:'Discovery+',date:nextWd(1,12,0),venue:'Diverse toernooien',isLive:false});

  /* Live events section — award shows etc on streaming */
  /* These are placeholders; could be enhanced with an API later */
...
function applyContentFilter(items){
  ...
  if(activeFilter==='seasons')return items.filter(function(i){return i._type==='tv'&&(i._season||(i._epInfo&&i._epInfo.e===1));});
  return items;
}
...
/* ═══════════════════════════════════════════════════════════════
   INIT — Progressive loading: render UI as soon as first data
...
function init(){
  ...
  function tryRender(){
    ...
    if(!hasRendered){
      hasRendered=true;
      buildSvcBar();buildDateTabs();renderMain();
    }else{
      buildSvcBar();renderMain();
    }
  }

  /* Phase 1: Fast sources — Google Sheet + TMDB providers (small requests) */
  Promise.all([
    ...
    /* Phase 2a: Start enriching sheet items + render them immediately */
    ...
    /* Phase 2b: In parallel, fetch TMDB releases (fast, ~2-4s) */
    var tmdbReady=fetchTMDBReleases().catch(function(){return[];});

    /* Phase 2c: In parallel, fetch WM sources + releases (can be slow) */
    var wmReady=fetchWMSources().then(function(){
      return fetchWMReleases().catch(function(){return[];});
    }).catch(function(){return[];});

    /* As TMDB finishes, merge and re-render */
    tmdbReady.then(function(tmdbItems){
      return sheetReady.then(function(gsEnriched){
        allItems=mergeItems([],tmdbItems,gsEnriched);
        if(ls)ls.textContent='';
        tryRender();
        return gsEnriched;
      });
    ...
    /* When WM also finishes, do final merge */
    Promise.all([wmReady,tmdbReady,sheetReady]).then(function(res){
      var wmItems=res[0]||[];
      var tmdbItems=res[1]||[];
      var gsEnriched=res[2]||[];
      allItems=mergeItems(wmItems,tmdbItems,gsEnriched);
