...
function renderMain(){
  ...
  if(!items.length){main.innerHTML='<div class="empty-state"><div style="font-size:16px;margin-bottom:6px;font-weight:600">Geen releases</div><div style="font-size:13px;color:var(--t3)">'+(activeFilter?'Geen resultaten voor dit filter. Probeer een andere dag of filter.':'Geen nieuwe content op '+getDateLabel(selectedDate).toLowerCase()+'. Probeer een andere dag.')+'</div></div>';return;}
...
function setType(f){
  ...
  /* Slogan always stays visible — never hidden */
  ...
  else{dt.style.display='';sb.style.display='';if(fb)fb.style.display='';renderMain();}
}
...
function applyContentFilter(items){
  ...
  if(activeFilter==='episodes')return items.filter(function(i){return i._type==='tv'&&i._epInfo&&i._epInfo.s;});
  if(activeFilter==='seasons')return items.filter(function(i){return i._type==='tv'&&i._season;});
  if(activeFilter==='originals')return items.filter(function(i){return i._originType==='original';});
...
function toggleFilter(f){
  ...
  if(dd){if(activeFilter==='genre')dd.classList.add('open');else dd.classList.remove('open');}
  /* Reset genre button text */
  var gb=document.getElementById('genreBtn');
  if(gb&&!activeGenre){gb.childNodes[0].textContent='Genre ';}
  renderMain();
}
  ...
  if(filterBar){
    filterBar.addEventListener('click',function(e){
      ...
      if(gopt){
        ...
        document.querySelectorAll('.filter-chip').forEach(function(c){c.classList.toggle('active',c.getAttribute('data-filter')===activeFilter);});
        var gb=document.getElementById('genreBtn');
        if(gb){var gName=GENRE_LIST.find(function(g){return g.id===activeGenre;});gb.childNodes[0].textContent=(gName?gName.name:'Genre')+' ';}
