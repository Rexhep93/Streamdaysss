...
/* ── Favorites ── */
var favorites=[];
function loadFavorites(){try{favorites=JSON.parse(localStorage.getItem('sg_favorites')||'[]');}catch(e){favorites=[];}}
function saveFavorites(){try{localStorage.setItem('sg_favorites',JSON.stringify(favorites));}catch(e){}}
...
function toggleFavorite(item){
  ...
  if(idx>=0){favorites.splice(idx,1);saveFavorites();showToast('Verwijderd uit favorieten');return false;}
  favorites.push({id:item.id,title:item.title,img:item.img||imgCache[item.id]||'',_type:item._type,_key:item._key,_src:item._src,tmdb_id:item.tmdb_id||null});
  saveFavorites();
  if(getSettingBool('notif_releases')&&'Notification' in window&&Notification.permission!=='granted'&&Notification.permission!=='denied'){
    if(confirm('Wil je meldingen ontvangen voor nieuwe releases van "'+item.title+'"?'))Notification.requestPermission();
  }
  showToast('Toegevoegd aan favorieten');return true;
}
...
function applyPosterSize(){document.querySelector('.app').classList.toggle('poster-large',posterSize==='large');}
...
function fetchTMDBReleases(){
  ...
  function makeItem(m,prov,id,type,overrideDate,epInfo,monetization){var date=overrideDate||(type==='movie'?(m.release_date||''):(m.first_air_date||''));if(!date||!m.id)return null;return{id:'tmdb-'+m.id+'-'+id+(overrideDate?'-'+overrideDate:'')+(monetization&&monetization!=='flatrate'?'-'+monetization:''),title:m.title||m.name||m.original_title||m.original_name||'',img:m.poster_path?TMDB_IMG+m.poster_path:null,_type:type,_date:date,_src:{name:prov.name,logo_100px:prov.logo},_style:{color:prov.color,text:prov.text},_key:providerKey(prov.name),_originType:detectOrigin(prov.name),_source:'tmdb',_providerId:Number(id),tmdb_id:m.id,user_rating:m.vote_average||0,overview:m.overview||'',_epInfo:epInfo||null,_genres:m.genre_ids||[],_monetization:monetization||'flatrate'};}
  ...
  return Promise.all([tmdbPages('/movie/now_playing',{language:'nl-NL',region:'NL'},5),tmdbPages('/tv/on_the_air',{language:'nl-NL'},5)]).then(function(res){
    ...
    return epBatch.then(function(){
      ...
      var bw=Promise.resolve();for(var i=0;i<checks.length;i+=10){(function(batch){bw=bw.then(function(){return Promise.all(batch.map(function(m){return tmdb('/'+(m._isTV?'tv':'movie')+'/'+m.id+'/watch/providers').then(function(pd){
        var nl=pd&&pd.results&&pd.results.NL||{};
        var flat=nl.flatrate||[];var rent=nl.rent||[];var buy=nl.buy||[];
        flat.forEach(function(p){var prov=TMDB_NL_PROVIDERS[p.provider_id];if(!prov)return;var it=makeItem(m,prov,p.provider_id,m._isTV?'tv':'movie',m._isTV?m._epDate:null,m._isTV?m._epInfo:null,'flatrate');if(it)items.push(it);});
        rent.concat(buy).forEach(function(p){var prov=TMDB_NL_PROVIDERS[p.provider_id];if(!prov)return;var it=makeItem(m,prov,p.provider_id,m._isTV?'tv':'movie',m._isTV?m._epDate:null,m._isTV?m._epInfo:null,'rent');if(it)items.push(it);});
      }).catch(function(){});}));});})(checks.slice(i,i+10));}
      return bw;
    });
  }).then(function(){
...
function mergeItems(wm,tm,gs){
  ...
  /* Google Sheet: TV series added for weekly episodes, BUT skip if API
     already has the exact same title+provider+date (prevents duplicates
     like "Vladimir" showing twice on the same day). */
  (gs||[]).forEach(function(i){
    if(!i._date||!i.title)return;var tl=(i.title||'').toLowerCase().trim();
    if(i._type==='tv'){
      var dateKey=tl+'|'+i._key+'|'+i._date;
      /* If API already has this title+provider+date → skip entirely */
...
function sportEventsToItems(evs){
  return (evs||[]).map(function(ev){
    ...
    var dateISO=ev.date.toISOString().slice(0,10);
    var timeStr=ev.date.toLocaleTimeString('nl-NL',{hour:'2-digit',minute:'2-digit'});
    return {
      id:ev.id,title:ev.title,img:logo||null,
      _type:'live',_date:dateISO,
      _src:{name:prov?prov.name:ev.streamer,logo_100px:prov?null:null},
      _style:{color:prov?prov.color:'#444',text:'#fff'},
      _key:prov?prov.key:(ev.streamer||'').toLowerCase(),
      _originType:'live',_source:'sport',
      overview:(ev.subtitle||'')+(ev.venue?' · '+ev.venue:'')+' · '+timeStr,
      user_rating:0,
      _sportCategory:ev.sport||'',_sportSub:ev.subtitle||'',_sportTime:timeStr,
      _isLiveNow:ev.isLive||false
    };
  });
}
...
function getSportCategory(item){
  ...
  if(s.indexOf('voetbal')!==-1||sub.indexOf('league')!==-1||sub.indexOf('liga')!==-1||sub.indexOf('eredivisie')!==-1)return 'voetbal';
  if(s.indexOf('ufc')!==-1||s.indexOf('mma')!==-1)return 'ufc';
  if(s.indexOf('wwe')!==-1||s.indexOf('wrestling')!==-1)return 'wwe';
  if(s.indexOf('f1')!==-1||sub.indexOf('formule')!==-1)return 'f1';
  if(s.indexOf('event')!==-1||s.indexOf('award')!==-1||s.indexOf('show')!==-1)return 'live_other';
  return 'sport_other';
}
...
function buildFilterChips(){
  ...
  if(streamType==='all'){
    ...
    html+='<button class="filter-chip'+(streamSubFilter==='streaming'?' active':'')+'" data-sf="streaming">Streaming</button>';
    html+='<button class="filter-chip'+(streamSubFilter==='rent'?' active':'')+'" data-sf="rent">Huren / Kopen</button>';
...
function applySubFilter(items){
  ...
  if(streamSubFilter==='streaming')return items.filter(function(i){return !i._monetization||i._monetization==='flatrate';});
  if(streamSubFilter==='rent')return items.filter(function(i){return i._monetization==='rent';});
  return items;
}

function renderStreamView(){
  /* Live events are now integrated as regular items in renderMain */
  document.getElementById('dateTabs').style.display=(streamType==='live')?'none':'';
  document.getElementById('svcBar').style.display=(streamType==='live')?'none':'';
  renderMain();
}

function renderMain(){
  var main=document.getElementById('main');main.innerHTML='';
  var pClass=posterSize==='large'?' poster-large':'';

  /* For 'live' streamType, render sport events as crow items */
  if(streamType==='live'){
    if(!sportCache){
      main.innerHTML='<div class="loading-screen"><div class="ld-spinner"></div><div class="ld-text">Live evenementen laden...</div></div>';
      fetchSportEvents().then(function(){renderMain();});
      return;
    }
    var evs=sportCache||[];
    /* Apply sport sub-filter */
    if(streamSubFilter){
      var sl=streamSubFilter.toLowerCase();
      evs=evs.filter(function(e){
        var s=(e.sport||'').toLowerCase();var sub=(e.subtitle||'').toLowerCase();
        if(sl==='voetbal')return s.indexOf('voetbal')!==-1||sub.indexOf('league')!==-1||sub.indexOf('liga')!==-1||sub.indexOf('eredivisie')!==-1;
        if(sl==='ufc')return s.indexOf('ufc')!==-1||s.indexOf('mma')!==-1;
        if(sl==='wwe')return s.indexOf('wwe')!==-1||s.indexOf('wrestling')!==-1;
        if(sl==='f1')return s.indexOf('f1')!==-1||sub.indexOf('formule')!==-1;
        if(sl==='sport_other')return ['voetbal','ufc','mma','wwe','wrestling','f1'].every(function(x){return s.indexOf(x)===-1&&sub.indexOf(x)===-1;})&&s.indexOf('event')===-1&&s.indexOf('award')===-1;
        if(sl==='live_other')return s.indexOf('event')!==-1||s.indexOf('award')!==-1||s.indexOf('show')!==-1;
        return true;
      });
    }
    if(!evs.length){
      main.innerHTML='<div class="empty-state"><div style="font-size:16px;margin-bottom:6px;font-weight:600">Geen live evenementen</div><div style="font-size:13px;color:var(--t3);margin-bottom:14px">Geen aankomende evenementen gevonden.</div><div class="empty-cta-row"><button class="empty-cta-btn" onclick="streamType=\'all\';streamSubFilter=null;buildFilterChips();renderStreamView();">Bekijk alles</button></div></div>';
      return;
    }
    /* Group by streamer */
    var svcGroups={};var svcOrder=[];
    evs.forEach(function(ev){
      var k=(ev.streamer||'overig').toLowerCase().replace(/\s+/g,' ');
      if(!svcGroups[k]){
        var prov=matchProvider(ev.streamer);
        svcGroups[k]={name:ev.streamer||'Live',logo:prov?null:null,color:prov?prov.color:'#444',items:[]};
        /* Get logo from TMDB providers */
        Object.keys(TMDB_NL_PROVIDERS).forEach(function(pid){var pr=TMDB_NL_PROVIDERS[pid];if(pr&&providerKey(pr.name)===providerKey(ev.streamer)&&pr.logo)svcGroups[k].logo=pr.logo;});
        svcOrder.push(k);
      }
      svcGroups[k].items.push(ev);
    });
    svcOrder.forEach(function(key,idx){
      var g=svcGroups[key];var sec=document.createElement('div');sec.className='svc-section';sec.style.animationDelay=(idx*0.04)+'s';
      ...
      sec.innerHTML='<div class="svc-hdr" data-key="'+key+'">'+logoHtml+'<div class="svc-name">'+g.name+'</div><div class="svc-cnt">'+g.items.length+' evenement'+(g.items.length>1?'en':'')+'</div><svg class="svc-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></div><div class="svc-body'+pClass+'">'+g.items.map(function(ev){return sportCrowHtml(ev);}).join('')+'</div>';
      main.appendChild(sec);
    });
    return;
  }

  /* Normal film/series view */
  ...
  if(!items.length){
    ...
    main.innerHTML='<div class="empty-state"><div style="font-size:16px;margin-bottom:6px;font-weight:600">Geen releases</div><div style="font-size:13px;color:var(--t3);margin-bottom:14px">Geen nieuwe content op '+getDateLabel(selectedDate).toLowerCase()+'.</div>'+ctas+'</div>';return;
  }
  var svcGroups={};var svcOrder=[];
  ...
  svcOrder.forEach(function(key,idx){
    ...
    sec.innerHTML='<div class="svc-hdr" data-key="'+key+'">'+logoHtml+'<div class="svc-name">'+g.name+'</div><div class="svc-cnt">'+parts.join(' · ')+'</div><svg class="svc-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></div><div class="svc-body'+pClass+'">'+g.items.map(function(item){return crowHtml(item);}).join('')+'</div>';
    main.appendChild(sec);
  });
}

/* Sport event as regular crow item */
function sportCrowHtml(ev){
  var title=(ev.title||'').replace(/'/g,"&#39;").replace(/"/g,'&quot;');
  var sid=String(ev.id).replace(/['"\\]/g,'');
  var logo=getSportLogo(ev.sport,ev.subtitle);
  var timeStr=ev.isLive?'<span style="color:var(--red);font-weight:700">LIVE</span>':formatSportDate(ev.date);
  var info=SPORT_STREAMERS[ev.streamer]||{bg:'rgba(100,100,100,0.85)'};
  return '<div class="crow sport-crow" data-sport-id="'+sid+'"><div class="crow-poster" style="display:flex;align-items:center;justify-content:center;background:#fff">'+(logo?'<img src="'+logo+'" alt="" style="width:32px;height:32px;object-fit:contain" onerror="this.style.display=\'none\'">':'<div class="crow-fb" style="font-size:10px">'+ev.sport+'</div>')+'</div><div class="crow-info"><div class="crow-title">'+title+'</div><div class="crow-meta">'+ev.subtitle+(ev.venue?' · '+ev.venue:'')+'</div><div class="crow-meta" style="margin-top:2px">'+timeStr+'</div></div><div class="crow-badge serie" style="background:'+info.bg+';color:#fff;font-size:11px">'+ev.streamer+'</div></div>';
}
...
function crowHtml(item){
  ...
  if(item._type==='tv'){
    ...
    if(hasEpInfo){
      ...
      var airedEps=ep.airedEpisodes||null;
      var completeDrop=ep.isCompleteDrop||false;
      var isFirstEp=ep.e===1;
      if(completeDrop&&totalEps&&airedEps>=totalEps){
        /* All episodes dropped at once → only "Nieuw seizoen" */
        epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+ep.s+')</div>';
      }else if(isFirstEp||hasSeason){
        /* Partial release: new season + episode info */
        epBadge='<div class="crow-ep-badge season">Nieuw seizoen (S'+ep.s+')</div>';
        if(totalEps){
          epBadge+='<div class="crow-ep-badge">Aflevering '+airedEps+' van '+totalEps+'</div>';
        }else{
          epBadge+='<div class="crow-ep-badge">Nieuwe aflevering</div>';
        }
      }else{
        /* Regular weekly episode */
        if(totalEps&&airedEps){
          epBadge='<div class="crow-ep-badge">Aflevering '+airedEps+' van '+totalEps+'</div>';
        }else{
          epBadge='<div class="crow-ep-badge">Nieuwe aflevering</div>';
        }
  ...
  return '<div class="crow" data-id="'+sid+'"><div class="crow-poster">'+(poster?'<img src="'+poster+'" alt="'+title+'" loading="lazy" onerror="this.parentElement.innerHTML=\'<div class=crow-fb>'+title.replace(/'/g,'').replace(/"/g,'')+'</div>\'">':'<div class="crow-fb" id="fb-'+sid+'">'+title+'</div>')+'</div><div class="crow-info"><div class="crow-title">'+title+'</div>'+epBadge+'<div class="crow-meta">'+tl+' - '+sn+alsoOnHtml+'</div></div><div class="crow-badge '+cls+'">'+tl+'</div></div>';
}
...
function updateFavBtn(){
  var btn=document.getElementById('favBtn');var lbl=document.getElementById('favBtnLabel');
  if(!btn||!currentModalItem){if(btn)btn.style.display='none';return;}
  btn.style.display='flex';var fav=isFavorite(currentModalItem.id);
  if(lbl)lbl.textContent=fav?'Verwijder favoriet':'Toevoegen aan favorieten';
  var ico=btn.querySelector('.fav-icon');if(ico)ico.setAttribute('fill',fav?'currentColor':'none');
...
function openSportModal(ev){
  var overlay=document.getElementById('overlay');var si=document.getElementById('si');var sd=document.getElementById('sd');
  var wb=document.getElementById('wb');var shareBtn=document.getElementById('shareBtn');var ratingsRow=document.getElementById('ratingsRow');
  var favBtn=document.getElementById('favBtn');if(favBtn)favBtn.style.display='none';
  currentModalItem=null;
  var logo=getSportLogo(ev.sport,ev.subtitle);
  var info=SPORT_STREAMERS[ev.streamer]||{bg:'rgba(100,100,100,0.85)'};
  var timeStr=ev.isLive?'LIVE NU':formatSportDate(ev.date);
  si.innerHTML='<div class="sheet-header"><div class="sp" style="display:flex;align-items:center;justify-content:center;background:#fff">'+(logo?'<img src="'+logo+'" alt="" style="width:50px;height:50px;object-fit:contain">':'')+'</div><div class="sinf"><div class="ssvc" style="color:'+info.bg+'">'+ev.streamer+'</div><div class="stitle">'+ev.title+'</div><div class="smeta">'+ev.subtitle+(ev.venue?' · '+ev.venue:'')+'</div><div class="stags"><span class="stag">'+timeStr+'</span><span class="stag">'+ev.sport+'</span></div></div></div>';
  sd.textContent='';ratingsRow.style.display='none';wb.style.display='none';
...
function renderTop10(){
  ...
  sec.innerHTML='<div class="top10-title">Top 10 trending '+(top10Period==='day'?'vandaag':'deze week')+'</div><div class="top10-sub">Meest bekeken op streaming in NL</div><div class="t10-tabs" id="t10tabs"></div><div id="t10list" class="t10-list"><div class="t10-loading"><div class="ld-spinner" style="margin:0 auto 8px"></div>Laden...</div></div>';
...
function doSearch(q){
  ...
  Promise.all([tmdb('/search/movie',{query:q,language:'nl-NL',region:'NL'}).catch(function(){return{results:[]};}),tmdb('/search/tv',{query:q,language:'nl-NL',region:'NL'}).catch(function(){return{results:[]};})]).then(function(res){
    ...
    combined.sort(function(a,b){var aL=a._source!=='tmdb'||a._key!=='search'?1:0;var bL=b._source!=='tmdb'||b._key!=='search'?1:0;if(aL!==bL)return bL-aL;return(b.user_rating||0)-(a.user_rating||0);});
...
function fetchKijktip(){
  ...
  return fetch(GOOGLE_SHEET_KIJKTIP_URL).then(function(r){return r.text();}).then(function(csv){
    ...
    for(var i=lines.length-1;i>=1;i--){
      ...
      if(!titel)continue;
      best={titel:titel,tmdb_id:(ci.tmdb_id>=0?cols[ci.tmdb_id]:'').replace(/^["']|["']$/g,'').trim(),type:(ci.type>=0?cols[ci.type]:'movie').replace(/^["']|["']$/g,'').trim().toLowerCase(),tekst:(ci.tekst>=0?cols[ci.tekst]:'').replace(/^["']|["']$/g,'').trim(),datum:(ci.datum>=0?cols[ci.datum]:'').replace(/^["']|["']$/g,'').trim()};
      break;
    }
    kijktipCache=best;return best;
  }).catch(function(){return null;});
}
function renderKijktip(){
  var main=document.getElementById('main');main.innerHTML='';
  var sec=document.createElement('section');sec.className='kijktip-section';
  sec.innerHTML='<div class="kt-hdr"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--green)" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg><div class="kt-title">Kijktip van de week</div></div><div id="ktContent" class="kt-content"><div class="t10-loading">Laden...</div></div>';
  main.appendChild(sec);
  ...
  fetchKijktip().then(function(tip){
    var el=document.getElementById('ktContent');if(!el)return;
    if(!tip){el.innerHTML='<div class="kt-empty"><div class="kt-empty-title">Nog geen kijktip</div><div class="kt-empty-sub">Binnenkort verschijnt hier een nieuwe aanbeveling.</div></div>';return;}
...
function renderSettings(){
  ...
  '<div class="set-row set-link" id="setFavLink"><span>Mijn favorieten</span><span class="set-cnt">('+favorites.length+')</span><span class="set-arrow">&rsaquo;</span></div>'+
  ...
  '<div class="set-row"><span>Versie</span><span class="set-val">1.0.0</span></div>'+
  '<div class="set-row"><span>Data</span><span class="set-val">TMDB / JustWatch</span></div>'+
  ...
  sec.querySelector('#posterToggle').addEventListener('click',function(e){var btn=e.target.closest('.set-tog');if(!btn)return;var v=btn.getAttribute('data-v');setSetting('posterSize',v);posterSize=v;sec.querySelectorAll('#posterToggle .set-tog').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-v')===v);});showToast('Posterweergave: '+(v==='large'?'Groot':'Compact'));
    /* Update all currently visible svc-body elements */
    document.querySelector('.app').classList.toggle('poster-large',v==='large');
  });
...
function renderFavorites(){
  var main=document.getElementById('main');main.innerHTML='';
  var sec=document.createElement('section');sec.className='settings-section';
  var html='<div class="set-back" id="setBack">&larr; Instellingen</div><div class="set-title">Mijn favorieten</div>';
  if(!favorites.length){html+='<div class="kt-empty" style="padding:40px 20px"><div class="kt-empty-title">Geen favorieten</div><div class="kt-empty-sub">Voeg films en series toe via het hartje in de detail-sheet.</div></div>';}
  else{html+='<div class="fav-list">'+favorites.map(function(f){
...
function renderSettingsPage(page){
  ...
  if(page==='deletedata')content='<p>Wil je al je gegevens verwijderen?</p><button class="retry-btn" onclick="localStorage.clear();sessionStorage.clear();showToast(\'Data verwijderd\');setTimeout(function(){location.reload();},500);">Alle data verwijderen</button>';
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  loadFavorites();posterSize=getSetting('posterSize','compact');applyPosterSize();if(posterSize==='large')document.querySelector('.app').classList.add('poster-large');
  ...
  var favBtn=document.getElementById('favBtn');if(favBtn)favBtn.addEventListener('click',function(){if(currentModalItem){toggleFavorite(currentModalItem);updateFavBtn();}});
  ...
  document.getElementById('main').addEventListener('click',function(e){
    ...
    /* Sport crow click → open simple info sheet */
    var scrow=e.target.closest('.sport-crow');if(scrow){
      var sid=scrow.getAttribute('data-sport-id');
      var ev=(sportCache||[]).find(function(x){return String(x.id)===sid;});
      if(ev)openSportModal(ev);
    }
  });
...
function init(){
  ...
  Promise.all([fetchTMDBProviders().catch(function(){}),fetchGoogleSheet()]).then(function(setup){
    ...
    Promise.all([wmReady,tmdbReady,sheetReady]).then(function(res){
      ...
      console.log('[StreamGids] '+allItems.length+' items geladen');
      if(ls)ls.textContent='';tryRender();enrichMissingPosters();
      /* Pre-fetch sport events and merge into allItems as type 'live' */
      fetchSportEvents().then(function(sportEvs){
        if(!sportEvs||!sportEvs.length)return;
        var sportItems=sportEvs.map(function(ev){
          var logo=getSportLogo(ev.sport,ev.subtitle);
          var streamerMatch=matchProvider(ev.streamer);
          return{
            id:ev.id,title:ev.title,img:logo||null,
            _type:'live',_date:ev.date.toISOString().slice(0,10),
            _src:{name:ev.streamer||'Streaming',logo_100px:streamerMatch&&TMDB_NL_PROVIDERS[Object.keys(TMDB_NL_PROVIDERS).find(function(pid){return TMDB_NL_PROVIDERS[pid].name===streamerMatch.name;})]?TMDB_NL_PROVIDERS[Object.keys(TMDB_NL_PROVIDERS).find(function(pid){return TMDB_NL_PROVIDERS[pid].name===streamerMatch.name;})].logo:null},
            _style:{color:streamerMatch?streamerMatch.color:'#444',text:'#fff'},
            _key:streamerMatch?streamerMatch.key:(ev.streamer||'').toLowerCase(),
            _originType:'live',_source:'sport',
            overview:ev.subtitle+(ev.venue?' · '+ev.venue:''),
            user_rating:0,_sportData:ev
          };
        });
        /* Add to allItems without duplicates */
        var existingIds={};allItems.forEach(function(i){existingIds[i.id]=true;});
        sportItems.forEach(function(si){if(!existingIds[si.id]){allItems.push(si);existingIds[si.id]=true;}});
        tryRender();
      }).catch(function(){});
    }).catch(function(e){
      console.error(e);
