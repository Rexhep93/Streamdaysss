...
function setType(f){
  ...
  var dt=document.getElementById('dateTabs');var sb=document.getElementById('svcBar');
...
function renderTop10(){
  ...
  sec.innerHTML='<div class="top10-title">Top 10 trending '+(top10Period==='day'?'vandaag':'deze week')+'</div><div class="top10-sub">Meest bekeken content wereldwijd</div><div class="t10-tabs" id="t10tabs"></div><div id="t10list" class="t10-list"><div class="t10-loading"><div class="ld-spinner" style="margin:0 auto 8px"></div>Laden...</div></div>';
  main.appendChild(sec);renderT10Tabs();
  fetchTop10(top10Period,top10Category).then(function(items){var el=document.getElementById('t10list');if(!el)return;if(!items.length){el.innerHTML='<div class="t10-loading">Geen data.</div>';return;}
    el.innerHTML=items.map(function(item,idx){var rank=idx+1;var isTV=item.media_type==='tv'||(!item.title&&item.name);var title=item.title||item.name||'?';var poster=item.poster_path?TMDB_IMG+item.poster_path:'';var year=(item.release_date||item.first_air_date||'').slice(0,4);var score=item.vote_average?Number(item.vote_average).toFixed(1):'';var tl=isTV?'Serie':'Film';var rc=rank<=3?'r'+rank:'';var safe=title.replace(/'/g,"&#39;");var mt=item.media_type||(isTV?'tv':'movie');
    return '<div class="t10-item" data-tmdb="'+item.id+'" data-mt="'+mt+'" data-title="'+safe+'"><div class="t10-rank '+rc+'">'+rank+'</div><div class="t10-poster">'+(poster?'<img src="'+poster+'" alt="'+safe+'" loading="lazy">':'')+'</div><div class="t10-info"><div class="t10-name">'+title+'<span class="t10-badge">'+tl+'</span></div><div class="t10-meta">'+year+(score?' - #'+rank+' trending':'')+'</div></div>'+(score?'<div class="t10-score">'+score+'</div>':'')+'</div>';}).join('');});
}
