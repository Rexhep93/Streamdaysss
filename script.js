...
/* ┌──────────────────────────────────────────────────────────────┐
   ...
   │  type = movie of tv                                         │
   └──────────────────────────────────────────────────────────────┘ */
var GOOGLE_SHEET_CSV_URL = '';  // <-- PLAK HIER JE GOOGLE SHEETS CSV URL
...
function fetchTMDBReleases(){
  ...
  function makeItem(m,prov,id,type,overrideDate){var date=overrideDate||(type==='movie'?(m.release_date||''):(m.first_air_date||''));if(!date||!m.id)return null;return{id:'tmdb-'+m.id+'-'+id+(overrideDate?'-'+overrideDate:''),title:m.title||m.name||m.original_title||m.original_name||'',img:m.poster_path?TMDB_IMG+m.poster_path:null,_type:type,_date:date,_src:{name:prov.name,logo_100px:prov.logo},_style:{color:prov.color,text:prov.text},_key:providerKey(prov.name),_originType:detectOrigin(prov.name),_source:'tmdb',_providerId:Number(id),tmdb_id:m.id,user_rating:m.vote_average||0,overview:m.overview||''};}
  ...
  return Promise.all([tmdbPages('/movie/now_playing',{language:'nl-NL',region:'NL'},5),tmdbPages('/tv/on_the_air',{language:'nl-NL'},5)]).then(function(res){
    ...
    var epDateMap={};/* tmdb_id -> latest episode air_date */
    ...
    for(var ei=0;ei<tvShows.length;ei+=10){(function(batch){
      epBatch=epBatch.then(function(){return Promise.all(batch.map(function(show){
        return tmdb('/tv/'+show.id,{language:'nl-NL'}).then(function(detail){
          /* Check last_episode_to_air and next_episode_to_air */
          var dates=[];
          if(detail.next_episode_to_air&&detail.next_episode_to_air.air_date)dates.push(detail.next_episode_to_air.air_date);
          if(detail.last_episode_to_air&&detail.last_episode_to_air.air_date)dates.push(detail.last_episode_to_air.air_date);
          /* Use the most relevant date (closest to today) */
          var today=todayISO();
          var best=null;
          dates.forEach(function(d){
            if(!best)best=d;
            else if(Math.abs(new Date(d)-new Date(today))<Math.abs(new Date(best)-new Date(today)))best=d;
          });
    ...
    return epBatch.then(function(){
      var checks=res[0].slice(0,50).map(function(m){return Object.assign({},m,{_isTV:false});}).concat(tvShows.map(function(t){return Object.assign({},t,{_isTV:true,_epDate:epDateMap[t.id]||null});}));
      var bw=Promise.resolve();for(var i=0;i<checks.length;i+=10){(function(batch){bw=bw.then(function(){return Promise.all(batch.map(function(m){return tmdb('/'+(m._isTV?'tv':'movie')+'/'+m.id+'/watch/providers').then(function(pd){var flat=pd&&pd.results&&pd.results.NL&&pd.results.NL.flatrate||[];flat.forEach(function(p){var prov=TMDB_NL_PROVIDERS[p.provider_id];if(!prov)return;var epDate=m._isTV?m._epDate:null;var it=makeItem(m,prov,p.provider_id,m._isTV?'tv':'movie',epDate);if(it)items.push(it);});}).catch(function(){});}));});})(checks.slice(i,i+10));}
      return bw;
    });
  }).then(function(){
...
function enrichSheetItems(items){
  ...
  return next().then(function(){return items;});
}
...
function mergeItems(wm,tm,gs){
  ...
  var ts={};tm.forEach(function(i){if(!i._date||!i.title)return;var dk=i.tmdb_id+'|'+i._key;if(ts[dk])return;ts[dk]=true;var tk=(i.title||'').toLowerCase().trim()+'|'+i._key;if(seen[tk])return;seen[tk]=true;result.push(i);});
  /* Google Sheet items get priority — they fill gaps the APIs miss */
  (gs||[]).forEach(function(i){if(!i._date||!i.title)return;var gk=(i.title||'').toLowerCase().trim()+'|'+i._key+'|'+i._date;if(seen[gk])return;seen[gk]=true;/* Also check without date */var gk2=(i.title||'').toLowerCase().trim()+'|'+i._key;if(seen[gk2])return;seen[gk2]=true;result.push(i);});
  return result;
}
