...
function canonicalProviderLogo(key){
  /* Return original brand logo by key, ignoring variant names */
  var canonKey=(key||'').toLowerCase();
  for(var pid in TMDB_NL_PROVIDERS){
    var pr=TMDB_NL_PROVIDERS[pid];
    if(pr&&pr.name&&providerKey(pr.name)===canonKey&&pr.logo)return pr.logo;
  }
  return null;
}
...
function isInMyList(id){return myList.some(function(f){return f.id===id;});}
function addToMyList(item){
  if(!item||isInMyList(item.id))return false;
...
function renderMain(){
  ...
  svcOrder.forEach(function(key,idx){
    ...
    var canonLogo=g.logo||canonicalProviderLogo(key)||null;
  ...
  if(item._type==='live'){
    ...
    if(hasEpInfo){
      ...
      if(completeDrop&&totalEps&&airedEps>=totalEps){
        ...
        /* Regular weekly episode: always show season */
        var seasonPrefix='<div class="crow-ep-badge season">Seizoen '+ep.s+'</div>';
        if(totalEps&&airedEps){
          epBadge=seasonPrefix+'<div class="crow-ep-badge">Nieuwe aflevering</div>';
        }else{
          epBadge=seasonPrefix+'<div class="crow-ep-badge">Nieuwe aflevering</div>';
        }
      }
...
function renderMyList(){
  ...
  if(!myList.length){
    ...
    html+='<div class="fav-list">'+myList.map(function(f){
      ...
      var sn=(f._src&&f._src.name||'').replace(/Netherlands/gi,'').trim();
...
function renderTop10(){
  ...
  sec.innerHTML='<div class="top10-title">Top 10 trending '+(top10Period==='week'?'deze week':'vandaag')+'</div><div class="top10-sub">Meest bekeken op streaming in NL</div><div class="t10-tabs" id="t10tabs"></div><div id="t10list" class="t10-list"><div class="t10-loading"><div class="ld-spinner" style="margin:0 auto 8px"></div>Laden...</div></div>';
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  /* ── Pull to refresh ── */
  var ptrStart=0,ptrActive=false,ptrTriggered=false;
  var ptrEl=document.getElementById('ptrIndicator');
  document.addEventListener('touchstart',function(e){if(window.scrollY===0)ptrStart=e.touches[0].clientY;},{ passive:true });
  document.addEventListener('touchmove',function(e){
    if(!ptrStart)return;
    var dist=e.touches[0].clientY-ptrStart;
    if(dist>10&&window.scrollY===0){ptrActive=true;if(ptrEl){ptrEl.style.transform='translateY('+(Math.min(dist,80)-50)+'px)';ptrEl.style.opacity=Math.min(dist/60,1);ptrTriggered=dist>60;ptrEl.querySelector('.ptr-text').textContent=ptrTriggered?'Loslaten om te vernieuwen':'Trek naar beneden';}}
  },{ passive:true });
  document.addEventListener('touchend',function(){
    ...
    ptrStart=0;ptrActive=false;ptrTriggered=false;
  });
